...
export default function TempLand({ selectedImage }) {
  ...
  async function loadTextures() {
    ...
    console.log("1st load. uri=", uri1);

    textureLoader.load(uri1,

      (albedoAsset) => {
        console.log('albedoAsset=', albedoAsset);
    ...
    var uri2 = Asset.fromModule(require("../assets/glbTexture/emission.png")).uri;
    console.log('2nd load. uri2=', uri2);

    textureLoader.load( uri2,
      (emissiveAsset) => {
  ...
  useEffect(() => {
    const textureLoader = new THREE.TextureLoader();
    console.log(Asset.fromModule(require("../assets/glbTexture/universal.png")).uri)
    textureLoader.load(
    ...
    Promise.all([loadObjectModel()]).then(async (gltf) => {
      if (sceneRef.current) {
        ...
        object.traverse((child) => {
          if (child.isMesh) {
            // 텍스쳐 적용
            console.log(albedoTextures, emissiveTextures)
            child.material.map = albedoTextures.albedo;
            child.material.emissiveMap = emissiveTextures.emissive;
