...
import { View, Image, ScrollView, Button, StyleSheet, TouchableWithoutFeedback, PanResponder, Animated, Dimensions } from 'react-native';
...
const imageWidth = (screenWidth - imageMargin * 4 - modalContentPadding * 2) / 3 - 4;; // 화면 가로 크기를 3으로 나눔
...
const ImageSlideUpPanel = () => {
  ...
  const handleImagePress = (id) => {
    if (id === selectedImageId) {
      ...
      navigation.navigate('Land', { selectedImage: images.find(image => image.id === id) });
    } else {
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (evt, gestureState) => {
        // 직접 pan 값을 업데이트
        if (gestureState.dy > 0) { // 오직 아래로 드래그할 때만 반응
          pan.setValue({ x: 0, y: gestureState.dy });
        } // y 값은 0 이상으로 제한
      },

      onPanResponderRelease: () => {
        if (pan.y._value > 200) { // 일정 거리 이상 드래그하면 모달 닫기
          Animated.timing(pan, {
  ...
  return (
    <View style={styles.container}>
      ...
      <Modal isVisible={isModalVisible} style={styles.modal} backdropOpacity={0} >
        <Animated.View style={[styles.modalContent, { transform: pan.getTranslateTransform() }]}>
          <View {...panResponder.panHandlers} style={styles.draggableBar} />
          <FlatList
            data={images}
